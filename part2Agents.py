...
import z3
from z3 import (BoolVal, Solver, Bool, Bools, Int, Ints, Or, Not, And, Implies, Distinct, If, Sum, sat)
...
class SpellCastingPuzzleWizard(WizardAgent):
    ...
    def react(self, state: GameState) -> GameAction:
        # if we already made a plan, keep following it
        if hasattr(self, "plan") and len(self.plan) > 0:
        ...
        # convert all stones into tuple locations
        original_types = {}
        ...
        def solveSpellCasting(finalFireStones, finalIceStones):
            ...
            # keep this only if your map border is supposed to be unused
            for row in range(rows):
                for col in range(cols):
                    if row == 0 or row == rows - 1 or col == 0 or col == cols - 1:
                        s.add(numEdges(isTouching(row, col)) == 0)
            ...
            # order variables help prevent disconnected loops
            order = {}
            ...
            # fire stone constraints
            for r, c in finalFireStones:
                left, right, up, down = directionalEdges(r, c)

                # fire stone must turn
                s.add(isTurn(r, c))

                # cells before/after the fire stone must be straight
            ...
            # ice stone constraints
            for r, c in finalIceStones:
                left, right, up, down = directionalEdges(r, c)
    
                # ice stone must go straight
                s.add(isStraight(r, c))

                # at least one neighbor before/after must turn
        ...
        def generateAssignments():
            stone_locations = list(all_stone_locations)
            assignments = []

            def backtrack(index, final_fire, final_ice, spell_changes, cost):
                if index == len(stone_locations):
                    assignments.append({
                        "cost": cost,
                        "fire": set(final_fire),
                        "ice": set(final_ice),
                        "spells": dict(spell_changes)
                    })
                    return

                loc = stone_locations[index]
                original = original_types[loc]

                if original == "fire":
                    # leave as fire, cost 0
                    final_fire.add(loc)
                    backtrack(index + 1, final_fire, final_ice, spell_changes, cost)
                    final_fire.remove(loc)

                    # change fire to ice, cost 10
                    final_ice.add(loc)
                    spell_changes[loc] = WizardSpells.FREEZE
                    backtrack(index + 1, final_fire, final_ice, spell_changes, cost + 10)
                    del spell_changes[loc]
                    final_ice.remove(loc)
                    ...
                    # change ice to fire, cost 15
                    final_fire.add(loc)
                    spell_changes[loc] = WizardSpells.FIREBALL
                    backtrack(index + 1, final_fire, final_ice, spell_changes, cost + 15)
                    del spell_changes[loc]
                    final_fire.remove(loc)

                else:
                    # neutral to ice, cost 10
                    final_ice.add(loc)
                    spell_changes[loc] = WizardSpells.FREEZE
                    backtrack(index + 1, final_fire, final_ice, spell_changes, cost + 10)
                    del spell_changes[loc]
            ...
            assignments.sort(key=lambda assignment: assignment["cost"])
            return assignments

        def addSpells(moves, path, spell_changes):
            plan = []
            already_cast = set()

            # if the start tile needs a spell, cast before moving
        ...
        # try cheapest spell assignments first
        assignments = generateAssignments()

        for assignment in assignments:
            result = solveSpellCasting(assignment["fire"], assignment["ice"])
        ...
        raise Exception("No spell assignment could solve this Masyu puzzle.")
